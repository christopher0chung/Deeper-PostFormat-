...
public class Deeper_ServicesLocator {
    ...
    public static Deeper_ServicesLocator instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                _instance.controllerToCharacter.Add(ControllersEnum.C0, CharactersEnum.Ops);
                _instance.controllerToCharacter.Add(ControllersEnum.C1, CharactersEnum.Doc);

                _instance.characterToController.Add(CharactersEnum.Ops, ControllersEnum.C0);
                _instance.characterToController.Add(CharactersEnum.Doc, ControllersEnum.C1);
            }
            return _instance;
        }
    }
    ...
    {
        return Deeper_GameUpdateManager.instance.PauseState();
    }
    ...
    public Dictionary<ControllersEnum, CharactersEnum> controllerToCharacter = new Dictionary<ControllersEnum, CharactersEnum>();
    public Dictionary<CharactersEnum, ControllersEnum> characterToController = new Dictionary<CharactersEnum, ControllersEnum>();

    public CharactersEnum GetCharacter (ControllersEnum c)
    {
        CharactersEnum output;
        controllerToCharacter.TryGetValue(c, out output);
        return output;
    }

    public ControllersEnum GetController (CharactersEnum c)
    {
        ControllersEnum output;
        characterToController.TryGetValue(c, out output);
        return output;
    }

    public int GetInt (CharactersEnum c)
    {
        ControllersEnum output;
        characterToController.TryGetValue(c, out output);
    ...
    #region Controllables References
        private GameObject Sub_GO;
        private GameObject Doc_GO;
        private GameObject Ops_GO;

        public GameObject GetRef (CharactersEnum c)
        {
            if (c == CharactersEnum.DANI)
            {
                if (Sub_GO == null)
                {
                    Sub_GO = GameObject.FindGameObjectWithTag("Sub");
                }
                return Sub_GO;
            }

            else if (c == CharactersEnum.Doc)
            {
                if (Doc_GO == null)
                {
                    Doc_GO = GameObject.FindGameObjectWithTag("Doc");
                }
                return Doc_GO;
            }

            else if (c == CharactersEnum.Ops)
            {
                if (Ops_GO == null)
                {
                    Ops_GO = GameObject.FindGameObjectWithTag("Ops");
                }
                return Ops_GO;
            }
    ...
    #region Task Manager Reference

    private Deeper_TaskManager _tm;
    public Deeper_TaskManager TaskManager
    {
        get
        {
            if (_tm == null)
